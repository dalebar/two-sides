...
**Status (end session 7):** Kick LOCKED (v02, session 2). Perc LOCKED (v03, session 3). Tone LOCKED (v04, session 5) — MoogFF-resonant three-saw at F2 with pitch drift, resonance drift, and function-pause cutoff automation. Convolution: algorithmic IR approach attempted across v05, v05b, v05c. All three reported as sounding wrong (metallic chamber / ball-bearings / metal-can). Algorithmic-IR approach paused pending session 8's spectrogram workflow, which will let Claude see what the IR is actually doing rather than guessing from symptom descriptions. Architectural alternatives in reserve: FDN reverb in SC, sampled IR from a real space, or Option 3 (retire convolution, reframe session 6's ontological-asymmetry conceptually rather than spatially). Decision deferred until spectrogram evidence is in. v04 remains LOCKED throughout; asymmetric-convolution question is entirely in v05-and-beyond territory.
